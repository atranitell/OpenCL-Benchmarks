...
__kernel void conv2d(__global const float* in,
  ...
  float reg = 0;
  if (ih >= 0 && iw >= 0 && ih + FILTER_H - 1 < in_h && iw + FILTER_W - 1 < in_w) {
    for (int ic = 0; ic < IN_C; ++ic) {
      ...
      reg += filter[p_f + FILTER_W] * in[p_in + in_w * DILATION_W];
      reg += filter[p_f + FILTER_W + 1] * in[p_in + DILATION_W + in_w * DILATION_W];
      reg += filter[p_f + FILTER_W + 2] * in[p_in + 2 * DILATION_W + in_w * DILATION_W];
      reg += filter[p_f + 2 * FILTER_W] * in[p_in];
      reg += filter[p_f + 2 * FILTER_W + 1] * in[p_in + DILATION_W + 2 * in_w * DILATION_W];
      reg += filter[p_f + 2 * FILTER_W + 2] * in[p_in + 2 * DILATION_W + 2 * in_w * DILATION_W];
    }
  } else {
    for (int ic = 0; ic < IN_C; ++ic) {
      int p_in = ic * in_h * in_w + ih * in_w + iw;
