...
__kernel void conv2d(__read_only image3d_t in,
                     __global float* out,
  ...
  const sampler_t splr = CLK_NORMALIZED_COORDS_FALSE |
                         CLK_ADDRESS_CLAMP |
                         CLK_FILTER_NEAREST;
  ...
  for (int ic = 0; ic < IN_C; ++ic) {
    ...
    for (int fh = 0; fh < FILTER_H; ++fh) {
      for (int fw = 0; fw < FILTER_W; ++fw) {
        reg += filter[p_f + fh * filter_w + fw] * read_imagef(in, splr, (int4)(iw+fw, ih+fh, ic, 0)).x;
      }
    }
  }
